...
typedef g2o::BlockSolver<g2o::BlockSolverTraits<9,3> > BalBlockSover;

// set up the verticies and edges for the bundle adjustments
...
void SetSolverOptionsFromFlags(BALProblem* bal_problem, const BundleParams& params, g2o::SparseOptimizer* optimizer){
    BalBlockSover* solver_ptr;

    g2o::LinearSolver<BalBlockSover::PoseMatrixType>* linearSolver = 0;

    if (params.linear_solver == "dense_schur" ){
        linearSolver = new g2o::LinearSolverDense<BalBlockSover::PoseMatrixType>();
    }else if (params.linear_solver == "sparse_schur"){
        linearSolver = new g2o::LinearSolverCholmod<BalBlockSover::PoseMatrixType>();
        // AMD ordering, only needed for sparse cholesky solver
        dynamic_cast<g2o::LinearSolverCholmod<BalBlockSover::PoseMatrixType>* > (linearSolver)->setBlockOrdering(true);
    }

    solver_ptr = new BalBlockSover(linearSolver);
...
int main(int argc, char** argv){
    ...
    if(params.input.empty()){
        std::cout << "Usage: bundle_adjuster -input <path for datasets>";
        return 1;
    }
