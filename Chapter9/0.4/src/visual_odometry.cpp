...
namespace myslam {
    ...
    bool VisualOdometry::addFrame(myslam::Frame::Ptr frame) {
        switch (state_) {
            case INITIALIZING: {
                state_ = OK;
                curr_ = ref_ = frame;
                // extract features from first frame and add them into the map
                extractKeyPoints();
                computeDescriptors();
    ...
    void VisualOdometry::poseEstimationPnP() {
        ...
        // use bundle adjustment to optimize the pose
    ...
    void VisualOdometry::optimizeMap() {
        // remove the hardly seem and no visible points
        for (auto iter = map_->map_points_.begin(); iter != map_->map_points_.end(); ){
            if (!curr_->isInFrame(iter->second->pos_) ){
                iter = map_->map_points_.erase(iter);
                continue;
            }
            float matche_ratio = float(iter->second->matched_times_)/iter->second->visible_times_;
            if (matche_ratio < map_point_erase_ratio_ ){
                iter = map_->map_points_.erase(iter);
                continue;
            }
